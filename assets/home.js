...
(function () {
  ...
  function crewBoarding(host, fin) {
    ...
    var deckY, deckW, hullH, shipL, lampLen, SC = 1;   // SC: 갑판 장면 배율 — 배(버튼)가 폭에 따라 커지므로 크루·강아지도 같은 비율로 키운다(기준 hullH 51px)                               // 갑판 y·배 폭·배 높이·배 왼쪽(뱃머리) x·등 빛 길이 — 캔버스 좌표
    function size() {
      ...
      SC = Math.max(1, Math.min(1.6, hullH / 51));
    ...
    function drawDog(t) {
      ...
      ctx.save(); ctx.translate(pos[0], pos[1] - rise * SC); ctx.rotate(sh.a); ctx.scale(SC, SC);
    ...
    function frame(now) {
      ...
      for (k = 0; k < N; k++) {
        ...
        var pos = tf(spot(p2), deckY), ph2 = p2.h * SC * (.55 + .45 * a);
