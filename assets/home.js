...
(function () {
  ...
  function crewBoarding(host, fin) {
    ...
    /* 빛은 크루와 1:1인 14줄기 — 크루의 별자리(별 14개)와 수를 맞췄다(2026-09-19 사용자 "그래야 말이 되지").
       `NS`는 사람이 되지 않고 선체에 스며들기만 하는 빛의 수로, 지금은 0이다. 다시 북적이게 하려면 여기만 올리면 된다
       (30 → 25 → 20 → 28로 조율했던 이력이 있다).
       ★ 굵기·속도·밝기는 크루의 빛과 똑같다: 앞서 가늘고 옅은 빛을 따로 뿌렸을 때 사용자가 "별로"라며 되돌렸다(커밋 304c598).
       시차만 크루 구간(140~last) 사이에 끼워 넣어 같은 시간에 두 배가 몰려들게 한다. */
    var NS = 0, S = [];                                                    // 0 = 빛이 곧 크루 (2026-09-19 사용자 "별자리도 14개니 몰려드는 빛도 14개로, 그래야 말이 되지")
    for (i = 0; i < NS; i++) {
  ...
  (function constellation() {
    ...
    function textBox() {                                                   // 눈썹~서브 카피 글자 상자(캔버스 좌표)
      ...
      for (var i = 0; i < els.length; i++) {
        if (!els[i].textContent.trim()) continue;                          // 편집 모드가 비운 문단은 건너뛴다
        var r = document.createRange(); r.selectNodeContents(els[i]); var rr = r.getBoundingClientRect();
        if (!rr.width) continue;
        var x0 = rr.left - hr.left, y0 = rr.top - hr.top, x1 = x0 + rr.width, y1 = y0 + rr.height;
        b = b ? { x0: Math.min(b.x0, x0), y0: Math.min(b.y0, y0), x1: Math.max(b.x1, x1), y1: Math.max(b.y1, y1) } : { x0: x0, y0: y0, x1: x1, y1: y1 };
      }
      return b;
    }
    function ray(cx, cy, c, s, x0, y0, x1, y1) {                          // 중심에서 (c,s) 방향으로 사각형 경계까지의 거리
      var tx = c > 1e-6 ? (x1 - cx) / c : c < -1e-6 ? (x0 - cx) / c : Infinity;
      var ty = s > 1e-6 ? (y1 - cy) / s : s < -1e-6 ? (y0 - cy) / s : Infinity;
      return Math.min(tx, ty);
    }
    function cross(a, b, c, d) {                                           // 선분 ab·cd 교차(끝점 공유는 제외)
      function o(p, q, r) { var v = (q.x - p.x) * (r.y - p.y) - (q.y - p.y) * (r.x - p.x); return v > 1e-9 ? 1 : v < -1e-9 ? -1 : 0; }
    ...
    function layout() {
      ...
      var b = textBox(); stars = []; edges = []; ok = false; cv.setAttribute('data-stars', 0); cv.setAttribute('data-edges', 0);
      if (!b || W < 200 || H < 300) return;
      var r = rng(20260919), cx = (b.x0 + b.x1) / 2, cy = (b.y0 + b.y1) / 2, mn = Math.min(W, H);
      var pad = Math.max(28, mn * .07), sx0 = b.x0 - pad, sy0 = b.y0 - pad, sx1 = b.x1 + pad, sy1 = b.y1 + pad;
      var nav = document.querySelector('.nav'), top = (nav ? nav.getBoundingClientRect().height : 72) + 16;
      var m = Math.max(12, mn * .035), dmin = mn * .12, pts = [];
      function fits(x, y) {
        if (x < m || x > W - m || y < top || y > H - m) return false;
        if (x > sx0 && x < sx1 && y > sy0 && y < sy1) return false;
        for (var i = 0; i < pts.length; i++) { var dx = pts[i].x - x, dy = pts[i].y - y; if (dx * dx + dy * dy < dmin * dmin) return false; }
        return true;
      }
      // 글자 둘레를 14개 방향으로 나눠 방향마다 하나씩 — 반지름은 무작위라 글자 곁의 별과 가장자리의 별이 섞인다
      var a0 = r() * 6.283, k, t;
      for (k = 0; k < N; k++) for (t = 0; t < 12; t++) {
        var a = a0 + (k + .5 + (r() - .5) * 1.3) / N * 6.283, c = Math.cos(a), sn = Math.sin(a);
        var r0 = ray(cx, cy, c, sn, sx0, sy0, sx1, sy1), r1 = ray(cx, cy, c, sn, m, top, W - m, H - m);
        if (r1 - r0 < 10) break;                                           // 이 방향엔 자리가 없다(폰의 좌우) — 아래에서 띠로 채운다
        var band = r() < .55 ? Math.pow(r(), 1.3) * .45 : .45 + r() * .55;   // 55%는 글자 곁, 45%는 가장자리 — 고른 반지름은 액자처럼 보였다
        var rad = r0 + 6 + band * (r1 - r0 - 6), x = cx + c * rad, y = cy + sn * rad;
        if (fits(x, y)) { pts.push({ x: x, y: y }); break; }
      }
      for (var g = 0; pts.length < N && g < 600; g++) {                    // 모자라면 위·아래 띠에서 채운다
        var up = r() < .5, yy = up ? top + r() * Math.max(1, sy0 - top) : sy1 + r() * Math.max(1, H - m - sy1), xx = m + r() * (W - 2 * m);
        if (fits(xx, yy)) pts.push({ x: xx, y: yy }); else if (g % 100 === 99) dmin *= .82;
      }
      if (pts.length < 8) return;                                          // 자리가 너무 없으면(짧은 화면) 그리지 않는다
      var idx = pts.map(function (_, i) { return i; }).sort(function () { return r() - .5; });
      pts.forEach(function (p, i) { var rank = idx.indexOf(i); p.mag = rank < 3 ? 2 : rank < 10 ? 1 : 0; p.ph = r() * 6.283; p.sp = 900 + r() * 900; p.a = 0; p.t0 = null; });
      // 선이 글자 상자를 지나면 안 된다 — 상자 네 변과의 교차로 본다
      var C = [{ x: sx0, y: sy0 }, { x: sx1, y: sy0 }, { x: sx1, y: sy1 }, { x: sx0, y: sy1 }];
      function blocked(p, q) { for (var e = 0; e < 4; e++) if (cross(p, q, C[e], C[(e + 1) % 4])) return true; return false; }
      // 최소 신장 트리(Prim) — 별자리답게 드문 선. 글자 상자를 넘지 않고는 이을 수 없으면 새 무리를 시작한다(폰의 위·아래 띠 = 별자리 둘)
      var inT = [0], rest = pts.map(function (_, i) { return i; }).slice(1), i, j, d;
      while (rest.length) {
        var best = null;
        for (i = 0; i < inT.length; i++) for (j = 0; j < rest.length; j++) {
          if (blocked(pts[inT[i]], pts[rest[j]])) continue;
          d = Math.hypot(pts[inT[i]].x - pts[rest[j]].x, pts[inT[i]].y - pts[rest[j]].y);
          if (!best || d < best.d) best = { a: inT[i], b: rest[j], d: d, j: j };
        }
        if (!best) { inT.push(rest.shift()); continue; }
        edges.push({ a: best.a, b: best.b, u: 0 }); inT.push(best.b); rest.splice(best.j, 1);
      }
      // 닫는 선 하나 — 트리에 없는 가장 짧은 짝, 다른 선과 교차하지 않고 트리 선 중간값의 1.35배 안일 때만
      var lens = edges.map(function (e) { return Math.hypot(pts[e.a].x - pts[e.b].x, pts[e.a].y - pts[e.b].y); }).sort(function (p, q) { return p - q; });
      var med = lens[lens.length >> 1], extra = null;
      for (i = 0; i < pts.length; i++) for (j = i + 1; j < pts.length; j++) {
        if (edges.some(function (e) { return (e.a === i && e.b === j) || (e.a === j && e.b === i); })) continue;
        d = Math.hypot(pts[i].x - pts[j].x, pts[i].y - pts[j].y);
        if (d > med * 1.35 || (extra && d >= extra.d)) continue;
        if (blocked(pts[i], pts[j]) || edges.some(function (e) { return cross(pts[i], pts[j], pts[e.a], pts[e.b]); })) continue;
        extra = { a: i, b: j, d: d };
      }
      if (extra) edges.push({ a: extra.a, b: extra.b, u: 0 });
      // 켜지는 순서 — 가장 왼쪽 별에서 시작해 트리를 따라 훑는다(선이 늘 켜진 별에서 뻗어 나가게)
      var adj = pts.map(function () { return []; }); edges.forEach(function (e) { adj[e.a].push(e.b); adj[e.b].push(e.a); });
      var start = 0; pts.forEach(function (p, i) { if (p.x < pts[start].x) start = i; });
      var seen = [], order = [];
      function dfs(i) { seen[i] = true; order.push(i); adj[i].slice().sort(function (p, q) { return pts[p].y - pts[q].y; }).forEach(function (j) { if (!seen[j]) dfs(j); }); }
      dfs(start);
      while (order.length < pts.length) { var nx = -1; pts.forEach(function (p, i) { if (!seen[i] && (nx < 0 || p.x < pts[nx].x)) nx = i; }); dfs(nx); }   // 떨어진 무리도 왼쪽부터 이어서
      pts.forEach(function (p, i) { p.k = order.indexOf(i); });
      edges.forEach(function (e) { if (pts[e.a].k > pts[e.b].k) { var tmp = e.a; e.a = e.b; e.b = tmp; } });   // a가 먼저 켜지는 별
      stars = pts; ok = true;
      cv.setAttribute('data-stars', pts.length); cv.setAttribute('data-edges', edges.length);   // 검증용 읽기 전용 표시(넓은 폭은 패널에서 캔버스가 안 보여 수치로 본다)
    }
    var relayout = null;
    function schedule() { clearTimeout(relayout); relayout = setTimeout(layout, 120); }
    layout(); window.addEventListener('resize', schedule);
    if (document.fonts && document.fonts.ready) document.fonts.ready.then(schedule);   // Pretendard가 늦게 오면 글자 상자가 달라진다
    var prev = 0;
    function frame(now) {
      requestAnimationFrame(frame);
      if (!ok) return;
      var vr = sec.getBoundingClientRect();
      if (vr.bottom < 0 || vr.top > innerHeight) { prev = now; return; }   // 화면 밖이면 그리지 않는다
      var dt = Math.min(64, now - (prev || now)); prev = now;
      var p = still ? 1 : progress(sec), q = Math.max(0, Math.min(1, (p - .06) / .76)), cnt = stars.length;
      stars.forEach(function (s) {                                         // 단어와 같은 진행 구간에서 순서대로 켜진다
        var on = q >= (s.k + .6) / cnt * .9;
        if (on && s.t0 == null) s.t0 = now; if (!on) s.t0 = null;
        s.a = still ? 1 : s.a + ((on ? 1 : 0) - s.a) * Math.min(1, dt / (on ? 240 : 320));
      });
      ctx.clearRect(0, 0, W, H);
      ctx.lineWidth = 1; ctx.lineCap = 'round';
      edges.forEach(function (e) {                                         // 선은 양끝 별이 켜진 뒤 먼저 켜진 별에서 뻗어 나간다
        var a = stars[e.a], b = stars[e.b], target = a.a > .6 && b.a > .6 ? 1 : 0;
        e.u = still ? 1 : e.u + (target - e.u) * Math.min(1, dt / (target ? 380 : 260));
        if (e.u < .01) return;
        var u = 1 - Math.pow(1 - e.u, 2), x = a.x + (b.x - a.x) * u, y = a.y + (b.y - a.y) * u;
        ctx.strokeStyle = 'rgba(255,196,140,' + ((.26 + .04 * Math.sin(now / 1700 + a.ph)) * Math.min(a.a, b.a)).toFixed(3) + ')';
        ctx.beginPath(); ctx.moveTo(a.x, a.y); ctx.lineTo(x, y); ctx.stroke();
      });
      stars.forEach(function (s) {
        if (s.a < .01) return;
        var age = s.t0 == null || still ? 9e9 : now - s.t0, bloom = age < 520 ? 1 + 1.1 * Math.pow(1 - age / 520, 2) : 1;   // 켜지는 순간 심이 잠깐 세진다 — 링은 없다
        var tw = still ? 1 : .9 + .1 * Math.sin(now / s.sp + s.ph);
        var core = [1.3, 1.9, 2.6][s.mag], halo = [8, 12, 17][s.mag], al = s.a * tw * [.7, .85, 1][s.mag];
        var gr = ctx.createRadialGradient(s.x, s.y, 0, s.x, s.y, halo * bloom);
        gr.addColorStop(0, 'rgba(255,176,110,' + (.5 * al).toFixed(3) + ')');
        gr.addColorStop(.4, 'rgba(237,109,32,' + (.16 * al).toFixed(3) + ')');
        gr.addColorStop(1, 'rgba(237,109,32,0)');
        ctx.fillStyle = gr; ctx.beginPath(); ctx.arc(s.x, s.y, halo * bloom, 0, 6.283); ctx.fill();
        ctx.fillStyle = 'rgba(255,245,232,' + al.toFixed(3) + ')'; ctx.beginPath(); ctx.arc(s.x, s.y, core * (1 + .6 * (bloom - 1)), 0, 6.283); ctx.fill();
      });
    }
    requestAnimationFrame(frame);
  })();
  /* 주의: 아래 별 캔버스 코드는 ?stars=0·모션 축소·편집 모드에서 바깥 IIFE를 통째로 return한다 — 그 뒤에 두는 코드는 그 경우 실행되지 않는다(별자리를 이 위로 옮긴 이유) */
