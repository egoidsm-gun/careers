...
(function () {
  ...
  /* ---------- 크루 카드 항행(2026-09-20) — 화면 한가운데에서 태어나 커지며 바깥으로 흘러 나간다 ----------
     ...
     2026-09-20 사용자 "너무 멀리서부터 사진이 온다, 조금 과하다, 덜 부담스럽게" — 여정을 줄였다(크기 6.8배→3.7배, 거리 13.6배→4.6배, 가장 밝은 카드도 .72→.50).
     카드는 소실점에서 점으로 태어나지 않고 중심에서 이미 17vw쯤 떨어진 자리에 떠올랐다가 조용히 흘러 나간다.
     ★ 한 번 여정을 너무 줄였더니(크기 2.5배) 카드 크기가 다 비슷해져 깊이감이 사라졌다 — 지금이 그 중간값이다.
     불투명도는 평평하게 두지 말 것: u에 비례해 올려야 가까운 한두 장만 도드라지고 나머지는 잦아들어 덜 부담스럽다.
     ★ 다만 바닥(.34)을 두지 않고 0에서 올리면 중간 카드가 통째로 안 보인다 — 크루 사진 원본이 실내 DSLR 컷이라 이미 어둡기 때문이다(실측).
  ...
  var P0 = .34, P1 = 1.55, S0 = .22, S1 = .82, LOOPS = 1.0;
  ...
  function flyShots(p) {
    ...
    for (var i = 0; i < n; i++) {
      ...
      // 들고 남을 대칭으로 재운다 — 여정이 짧아 카드가 화면을 완전히 못 벗어날 수도 있으므로 끝을 길게 빼서 튀지 않게 한다
      el.style.opacity = ((.34 + .66 * Math.pow(u, .6)) * Math.min(1, (1 - u) / .22) * .62).toFixed(3);
