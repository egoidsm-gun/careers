...
(function () {
  ...
  /* ---------- 크루 별자리 v3(2026-09-20 밤) — 글자를 둘러싸는, 한쪽이 열린 사슬 ----------
     ...
     **별자리 모양은 글자 상자에만 매여 있어 어느 화면에서도 닮은꼴이다.**
     ★ 2026-09-21 이 방식으로 바꿨다. 이전엔 '여백 정규화'(0~1 상자, -1~0 왼쪽/위 여백, 1~2 오른쪽/아래 여백)라 **창 높이에 따라 모양이 변했다** —
       사용자가 자기 크롬(세로 940: 탭바·북마크바가 먹음)과 내 캡처(1080)를 나란히 놓고 "별자리 위치도 달라"로 잡아냈다. 실측 별자리 세로폭 1080→898px / 940→770 / 780→623.
     화면이 좁아 다 안 들어가면 **글자 상자 중심 기준으로 전체를 같은 배율 k로 줄인다**(가로세로를 따로 누르지 않으므로 모양은 그대로). k=1이면 손대지 않은 것.
  ...
  var SKY_WIDE = [ // [ax, ay, 반지름] — 왼쪽 아래에서 시작해 왼쪽→위→오른쪽→아래로 도는 열린 사슬
    [-.413, 1.20, 1.6], [-.781, .62, 2.2], [-.367, .18, 1.5],
    [.05, -.426, 1.9], [.26, -.271, 1.5], [.48, -.481, 2.3], [.71, -.713, 3.6], [.93, -.349, 1.8],
    [1.322, .10, 2.1], [1.735, .55, 1.6], [1.276, 1.12, 1.5],
    [.86, 1.44, 2.0], [.60, 1.72, 1.4], [.36, 1.32, 1.9],
    [.20, 1.60, 1.2]
  ];
